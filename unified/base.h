...
// Test expression for the unbounded logical buffer tag.
template <typename Class>
using UnboundedBufferTest = typename Class::NOP_UNBOUNDED_BUFFER;

// Evaluates to true if Class is tagged with a nested type named
// NOP_UNBOUNDED_BUFFER.
template <typename Class>
using IsUnboundedBuffer = IsDetected<UnboundedBufferTest, Class>;
...
class BoundedWriter {
 public:
  ...
  constexpr Status<void> Write(const T* begin, const T* end) {
    ...
    const std::size_t length_bytes = (end - begin) * element_size;
    if (length_bytes > (size_ - index_))
      return ErrorStatus::WriteLimitReached;
...
class BufferWriter {
 ...
 private:
  ...
  // Writes a floating point element to the byte buffer. At the time of this
  // writing there isn't simple way to get the raw bytes of a floating point
  // type in a constexpr expression.
  void WriteElement(float value, std::size_t offset) {
    std::uint32_t raw_bytes;
    std::memcpy(&raw_bytes, &value, sizeof(value));
    WriteElement(raw_bytes, offset);
  }
  void WriteElement(double value, std::size_t offset) {
